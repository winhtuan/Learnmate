...
public class DashboardService(AppDbContext db) : IDashboardService
{
    public async Task<DashboardOverviewDto> GetDashboardOverviewAsync()
    {
        ...
        var usersLastMonth = await db.Users.CountAsync(u => u.CreatedAt >= startOfLastMonth && u.CreatedAt < startOfThisMonth);
        ...
        var classesThisMonth = await db.Classes.CountAsync(c => c.DeletedAt == null && c.CreatedAt >= startOfThisMonth);
        var classesLastMonth = await db.Classes.CountAsync(c => c.DeletedAt == null && c.CreatedAt >= startOfLastMonth && c.CreatedAt < startOfThisMonth);
        var classGrowth = classesLastMonth > 0
            ? Math.Round((classesThisMonth - classesLastMonth) / (double)classesLastMonth * 100, 1)
            : classesThisMonth > 0 ? 100.0 : 0.0;
